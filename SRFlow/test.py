# SRFlow因为模型过大，只能用CPU进行测试，否则会爆显存
import numpy as np
import torch
from torch.backends import cudnn
from tqdm import tqdm
import common.utils as utils
from SRFlow.SRFlow_model import SRFlowModel
...
def run(config_index):
    ...
    opt = options.parse('D:/Document/PycharmProj/SuperRestoration_Algorithm/SRFlow/SRFlow_DF2K_4X.yml', is_train=False)
    opt['gpu_ids'] = None
    opt = options.dict_to_nonedict(opt)

    model = SRFlowModel(opt, device)
